...
const ContentPage = () => {
  ...
  const completedOrders: OrderStatus[] = orders.filter(order => order.status === "completed");
  const preparingOrders: OrderStatus[] = orders.filter(order => order.status === "preparing");
        ...
        <ContainerStatus>
          ...
          <Orders>
            {preparingOrders && preparingOrders.map(order => {
              return (
                <Order key={order.id} status={order.status} >{order.name}</Order> 
              );
            })}
          </Orders>
        </ContainerStatus>
